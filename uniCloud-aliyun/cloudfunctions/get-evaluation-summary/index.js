...
exports.main = async (event = {}, context = {}) => {
  ...
  try {
    ...
    const teacherCourseReviews = buildTeacherCourseReviews(rows, teacherMap)
      .sort((a, b) => `${a.teacher_name} ${a.course_name}`.localeCompare(`${b.teacher_name} ${b.course_name}`));
...
function enrichEvaluation(row, offeringMap, courseMap, teacherMap) {
  const offering = firstMappedValue(offeringMap, [
    row.course_offering_id,
    row.courseOfferingId,
    row.offering_id,
    row.course_id
  ]) || {};
  const course = firstMappedValue(courseMap, [
    offering.course_id,
    offering.courseId,
    row.course_id,
    row.courseId
  ...
  const resolvedCourseId = offeringCourseId || String(course._id || course.id || '').trim() || (storedCourseId !== row.course_offering_id ? storedCourseId : '') || storedCourseId;
  const resolvedCourseOfferingId = String(row.course_offering_id || row.courseOfferingId || offering._id || offering.id || resolvedCourseId).trim();
  ...
  const courseName = courseTitle || (isDisplayableName(storedCourseName, resolvedCourseId, resolvedCourseOfferingId) ? storedCourseName : '') || resolvedCourseId || resolvedCourseOfferingId || 'Unnamed Course';
...
function buildTeacherCourseReviews(rows, teacherMap) {
  ...
  return Array.from(groups.values()).map((groupRows) => {
    ...
    const teacherName = teacher.name || teacher.teacher_no || first.teacher_names && first.teacher_names[0] || first.teacher_id || 'Unassigned Teacher';
    return {
