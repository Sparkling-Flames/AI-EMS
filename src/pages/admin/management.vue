<template>
  <view class="page">
    <PageHeader title="Admin Management" :display-name="session.displayName" :username="session.username">
    ...
    <view class="section">
      ...
      <view class="form-grid">
        ...
        <view v-if="accountForm.roleCode === 'teacher'" class="field">
          ...
          <picker
            :range="optionLabels.departments"
            :value="teacherDepartmentIndex"
            @change="teacherDepartmentIndex = Number($event.detail.value)"
          >
            <view class="picker-value"
              >{{ optionLabels.departments[teacherDepartmentIndex] || "Select department" }}
            </view>
          </picker>
        </view>
        <view v-if="accountForm.roleCode === 'teacher'" class="field">
    ...
    <view class="section">
      ...
      <view class="form-grid">
        ...
        <view class="field">
          ...
          <picker
            :range="optionLabels.majors"
            :value="courseMajorIndex"
        ...
        <view class="field">
          ...
          <view
            ...
            :class="{ 'readonly-error': availableSessionCount < Number(courseForm.totalSessions || 0) }"
          >
            {{ availableSessionCount }}
          </view>
          <text
            class="field-hint"
            :class="{ 'field-hint-error': availableSessionCount < Number(courseForm.totalSessions || 0) }"
          >
        ...
        <view class="slot-list">
          <view v-for="(slot, index) in courseForm.scheduleSlots" :key="slot.localId" class="slot-row">
            ...
            <view class="slot-field slot-classroom">
              ...
              <picker
                :range="optionLabels.classrooms"
                :value="classroomIndexForSlot(slot)"
            ...
            <button
              class="secondary-btn slot-remove-btn"
              :disabled="courseForm.scheduleSlots.length <= 1"
    ...
    <view class="section">
      ...
      <view class="timetable-grid">
        <view v-for="day in timetableWeekColumns" :key="day.weekday" class="timetable-day">
          ...
          <view
            v-for="sessionItem in day.sessions"
            :key="sessionItem._id"
            class="timetable-session"
      ...
      >
        <view class="course-actions">
          <button
            ...
            :loading="deletingCourseId === course.courseOfferingId"
    ...
    <view class="section">
      ...
      >
        <view class="account-actions">
          ...
          <button class="mini-btn danger" :disabled="!canDeleteAccount(account)" @click="deleteAccount(account)">
            Delete
          </button>
        </view>
      </DataCard>
...
export default {
  ...
  computed: {
    ...
    availableSessionCount() {
      return this.countAvailableSessions(
        this.courseForm.startDate,
        this.courseForm.endDate,
        this.normalizeScheduleSlots()
      );
    },
    sessionCapacityMessage() {
    ...
    filteredTimetableSessions() {
      ...
      const rows = this.groupTimetableSessions(
        (this.classSessions || []).filter(
          item => (!majorId || item.majorId === majorId) && (!gradeYear || Number(item.gradeYear || 0) === gradeYear)
        )
      );
  ...
  methods: {
    ...
    async saveAccount() {
      ...
      if (
        roleCode === "student" &&
        (!this.accountForm.studentNo ||
          !this.optionValue("majors", this.majorIndex) ||
      ...
      if (
        roleCode === "teacher" &&
        (!this.accountForm.teacherNo || !this.optionValue("departments", this.teacherDepartmentIndex))
      ) {
        uni.showToast({
    ...
    validateCourseForm(capacity, totalSessions, scheduleSlots) {
      ...
      if (
        ...
        !majorId ||
        !gradeYear ||
        !this.courseForm.teacherIds.length ||
        !capacity
      ) {
      ...
      for (let leftIndex = 0; leftIndex < scheduleSlots.length; leftIndex += 1) {
        for (let rightIndex = leftIndex + 1; rightIndex < scheduleSlots.length; rightIndex += 1) {
          ...
          if (
            ...
            this.timeRangesOverlap(left.startTime, left.endTime, right.startTime, right.endTime)
          ) {
            return "Schedule slots in the same course cannot overlap.";
          }
        }
      }
      const availableSessions = this.countAvailableSessions(
        this.courseForm.startDate,
        this.courseForm.endDate,
        scheduleSlots
      );
