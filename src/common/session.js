...
function readStoredSession() {
  const candidates = hasBrowserTabStorage()
    ? [
        readBrowserTabStorage(SESSION_KEY),
        readBrowserTabStorage(SESSION_BACKUP_KEY)
      ]
    : [
        readUniStorage(SESSION_KEY),
        readUniStorage(SESSION_BACKUP_KEY)
      ]
...
function normalizeSession(raw) {
  ...
  const role = normalizeRole([
    ...
    ...(Array.isArray(parsed.roles) ? parsed.roles.map(item => item && typeof item === 'object' ? item.code || item.role || item.name : item) : [])
  ])
